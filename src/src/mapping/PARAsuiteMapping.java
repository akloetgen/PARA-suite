	...
	public void executeMapping(int threads, String reference, String input,
		...
		if (!indexFile.exists()) {
			...
			MappingLogger.getLogger().info(
					"Creating BWA-index for reference file using BWA 0.7.8");
			executeCommand(bwaCommandsList, StreamRedirect.ERROR);
		...
		MappingLogger.getLogger().debug(
				"Convert PARA-suite mapping to SAM file");
		bwaCommandsList.clear();
		bwaCommandsList.add(parasuiteLocation + "bwa");
		...
		MappingLogger.getLogger().debug(
				"Time passed for PARA-suite algorithm: "
						+ calculatePassedTime()
